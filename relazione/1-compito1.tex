...
\begin{itemize}
    ...
    \item \textbf{Fattore di sconto}: $\gamma = 1$, riflette il fatto che il robot ugualmente valuta i premi futuri rispetto a quelli immediati. In combinazione con la struttura dei premi, garantisce che il cammino ottimo sia quello di lunghezza minima.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \quad\quad\quad\quad\quad\quad \textbf{then} $max\_q \gets -\infty$
    ...
    \item \quad\quad\quad\quad\quad\quad\quad\quad\quad $q \gets R(s, a) + \gamma \cdot V_{old}[r[s'], c[s']]$
    ...
    \item \quad\quad\quad\quad\quad\quad\quad $\Delta \gets \max(\Delta, |V_{old}[r, c] - V[r, c]|)$
...
\]
Nel caso peggiore in cui il cammino ottimo attraversi la quasi totalità delle celle libere della griglia allora $L=N^2$, altrimenti nel caso migliore abbiamo $L=1$. Di conseguenza possiamo affermare che la complessità temporale sia compresa, in generale, fra $\Omega(1)$ e $O(N^2)$.
