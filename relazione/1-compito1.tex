...
\begin{enumerate}
    \item \textbf{for} $r \gets 0$ \textbf{to} $N-1$
    \item \quad \textbf{do for} $c \gets 0$ \textbf{to} $N-1$
    \item \quad\quad \textbf{do} $V[r, c] \gets 0$
    \item \quad\quad\quad $\pi[r, c] \gets \text{NIL}$
    \item $\Delta \gets \infty$
    \item $k \gets 0$
    \item $max\_iter \gets N^2 + 1$
    \item \textbf{while} $\Delta \ge \epsilon$ \textbf{and} $k < max\_iter$
    \item \quad \textbf{do} $\Delta \gets 0$
    \item \quad\quad $k \gets k + 1$
    \item \quad\quad $V_{old} \gets V$
    \item \quad\quad \textbf{for} $r \gets 0$ \textbf{to} $N-1$
    \item \quad\quad\quad \textbf{do for} $c \gets 0$ \textbf{to} $N-1$
    ...
    \item \textbf{if} $\Delta \ge \epsilon$
    \item \quad \textbf{then error} "Nessuna convergenza: esistono celle libere da cui $G$ è irraggiungibile"
...
\begin{itemize}
    \item \textbf{Inizializzazione (righe 1--7):} I due cicli nidificati sulle righe $r$ e colonne $c$ scorrono tutte le $N \times N$ celle della matrice per azzerare $V$ e impostare $\pi$ a $\text{NIL}$. Ciascuna assegnazione richiede tempo $\Theta(1)$. La complessità dell'inizializzazione è dunque $\Theta(N^2)$.
    \item \textbf{Ciclo principale di convergenza (righe 8--26):} Sia $K$ il numero totale di iterazioni del ciclo \texttt{while} eseguite prima che la variazione massima rispetti il criterio di arresto $\Delta < \epsilon$. In ciascuna delle $K$ iterazioni vengono eseguite le seguenti operazioni:
    \begin{itemize}
        \item La copia $V_{old} \gets V$ (riga 11) duplica l'intera matrice $N \times N$ e costa quindi $\Theta(N^2)$;
        ...
        \item Per ogni cella ammissibile (non ostacolo e diversa da $G$), il ciclo interno (righe 18--23) valuta le $|\mathcal{A}| = 4$ direzioni cardinali. Il calcolo dello stato di destinazione $s' = \delta(s,a)$, la lettura di $R(s,a)$ e l'accesso in tabella a $V_{old}[r[s'], c[s']]$ avvengono in tempo costante $\Theta(1)$;
...
Il parametro $K$ non è indipendente dall'istanza. Per $\gamma = 1$ e con inizializzazione $V_0 \equiv 0$ si dimostra per induzione che, dopo $k$ iterazioni, la tabella $V$ contiene il valore esatto di tutte e sole le celle $s$ la cui distanza minima da $G$ (misurata in numero di mosse sul grafo delle celle libere) soddisfa $d(s,G) \le k$. Infatti alla prima iterazione ricevono il valore corretto $101 - 1$ esattamente le celle adiacenti a $G$, e al passo $k$ ogni cella a distanza $k$ legge dalla iterazione precedente il valore già corretto di un vicino a distanza $k-1$.

\noindent\\
Detta $\text{ecc}(G) = \max_{s \in \mathcal{S}} d(s, G)$ l'\textbf{eccentricità} del Goal nel grafo delle celle libere, ne segue che dopo $\text{ecc}(G)$ iterazioni tutti i valori sono corretti e che l'iterazione successiva produce $\Delta = 0$, arrestando il ciclo. Vale quindi l'uguaglianza
\[
...
\begin{itemize}
    ...
    \item \textbf{Ciclo di ricostruzione del cammino (righe 6--15):} Il ciclo \texttt{while} viene eseguito una volta per ciascuna cella che compone il cammino ottimo $P$. Sia $L$ la lunghezza del percorso estratto (in numero di passi), con $0 \le L \le N^2$:
    \begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Copia di lavoro $V_{old}[0..N-1, 0..N-1]$ (riga 11):} lo schema di aggiornamento \textbf{sincrono} richiede di conservare, per tutta la durata di ogni iterazione, l'intera tabella dei valori dell'iterazione precedente. Si tratta di una seconda matrice $N \times N$ di numeri reali, pari a ulteriori $\Theta(N^2)$ spazio. Questo è l'unico termine \textit{ausiliario} in senso stretto, cioè non richiesto dalla specifica del problema né dal formato dell'uscita: è precisamente il termine che la variante in-place del Compito 2 elimina;
    \item \textbf{Variabili scalari ausiliarie:} le variabili $r, c, \Delta, k, max\_iter, max\_q, best\_a, q, s, s'$ occupano uno spazio costante $\Theta(1)$.
\end{itemize}
